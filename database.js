 ...
 * typed query helpers, and retry logic.
 */
...
if (!SUPABASE_URL || !SUPABASE_SERVICE) {
  logger.error('Missing Supabase environment variables. Check SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.');
 ...
 * @param {Function} queryFn  — async fn that returns { data, error }
...
const executeWithRetry = async (queryFn, context, maxRetries = 3) => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      return await execute(queryFn, context);
    } catch (err) {
      lastError = err;
...
const dbHealthCheck = async () => {
  try {
    ...
    return !error || error.code === 'PGRST116'; // PGRST116 = no rows (healthy)
  } catch {
    return false;
  }
};

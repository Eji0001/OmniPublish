 ...
 * All routes require authentication. Users can only access their own posts.
 */
...
router.get('/', async (req, res) => {
  ...
  let query = supabase
    .from('posts')
    .select(`
      id, title, content, format, aspect_ratio, status,
      scheduled_at, published_at, created_at,
      post_platforms(platform, status, platform_post_url, published_at)
    `, { count: 'exact' })
...
router.get('/:id', async (req, res) => {
  const { data: post, error } = await supabase
    ...
    .eq('id', req.params.id)
    .eq('user_id', req.user.id)   // Row-level ownership
    .single();
...
router.post('/', validateBody('createPost'), async (req, res) => {
  ...
  if (error) {
    logger.error('Create post error', { err: error.message, userId: req.user.id });
    return res.status(500).json({ error: 'Failed to create post' });
  }

  // Insert platform targets
  if (platforms?.length) {
    ...
    await supabase.from('post_platforms').insert(targets);
  }

  // Attach media files
  if (mediaIds?.length) {
    await supabase.from('media_files').update({ post_id: post.id }).in('id', mediaIds).eq('user_id', req.user.id);
  }
...
/* ── POST /posts/adapt ──────────────────── */
// AI-powered platform content adaptation
router.post(
  '/adapt',
  aiRateLimiter,
  validateBody('adaptContent'),
  async (req, res) => {
...
router.get('/stats/overview', async (req, res) => {
  ...
  const counts = (totals || []).reduce((acc, p) => {
